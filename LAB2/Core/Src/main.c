...
void myPrintf2(int a, int b)
{
  ...
  if (LHS == RHS)
  {
    myPrintf1("Identity Verified: Yes\r\n");
  }
  else
  {
    myPrintf1("Identity Verified: No\r\n");
  } // check statements
}
...
{
  myPrintf1("---------------\r\n");
  myPrintf1("FUNCTION 3\r\n");
  int key = 2275142;    // student ID
  int shift = key % 95; // Calculate shift value
...
void myPrintf4(int a[2][2], int b[2][2])
{
  ...
  myPrintf1("Matrix A:\r\n");
  for (i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) myPrintf1("%d ", a[i][j]);
    myPrintf1("\r\n");
  }
