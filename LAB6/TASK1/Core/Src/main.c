...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      // Print frequency
      // NOTE: some toolchain configs disable float in printf. If printing floats
      // doesn't show decimals, either enable floats in printf settings or print integer Hz.
#if 1
      char msg[64];
...
#else
      // fallback integer print (Hz rounded)
      char msg[64];
      snprintf(msg, sizeof(msg), "Frequency: %lu Hz\r\n", (unsigned long)(frequency + 0.5));
      HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
#endif
      got_period = 0;
      HAL_Delay(200);
